const divisionData = [
  {
    divisionName: "1",
    oneMatchPerPair: true,
    numWinner: 1,
    numPrizeMoney: 3,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 2,
    players: [
      "O",
      "YeezyBoi",
      "Jerpidude",
      "Doge",
      "Huffulufugus",
      "PixelAndy",
      "dog",
      "Alex Thach",
      "Myles",
      "Cheez",
      "Tris",
      "Ben"
    ]
  },

  {
    divisionName: "2A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "Rahmations",
      "Redshurt",
      "Batfoy",
      "Scamper9",
      "e_flat7",
      "Trapzone"
    ]
  },

  {
    divisionName: "3A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["Sunny", "TegaMech", "xile", "nddk", "odinn", "Melon"]
  },
  {
    divisionName: "3B",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["Longboy", "Andrey", "Louie", "anicomi", "20Dan03", "SV"]
  },
  {
    divisionName: "3C",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["Eliwa", "Yusif", "Sofa", "NGC Man", "Mecex", "Tobi"]
  },

  {
    divisionName: "4A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["IanKeith", "teri", "Sargin", "Felinho", "Timothy Kim", "Santtu"]
  },
  {
    divisionName: "4B",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "evesy",
      "boxie",
      "Mr. Faq",
      "Night",
      "Koolio",
      "Jeez",
    ]
  },
  {
    divisionName: "4C",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["Mleczak", "Hezzuli", "TStorm", "Vince", "Armstrong", "Kaptan"]
  },

  {
    divisionName: "5A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "BirbWizard",
      "Tweak Beacon",
      "Grand Designs",
      "Insaniso",
      "VachetBeasty",
      "moodeuce",
    ]
  },
  {
    divisionName: "5B",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "Heist",
      "Morelando",
      "Blue Scuti",
      "Peyton",
      "Gunter",
      "OutCast",
    ]
  },

  {
    divisionName: "6A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["Tyler", "Maddy", "tourist106", "wide", "Cwiiis", "Moulis64"]
  },
  {
    divisionName: "6B",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "Elpailtem",
      "Kingsman",
      "TNISaint",
      "Maedhros",
      "Allenbot",
      "mattholland",
    ]
  },

  {
    divisionName: "7A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "sixfour",
      "Ivory",
      "scoopman",
      "Mario Lafleur",
      "Wolflord",
      "Świdr",
    ]
  },
  {
    divisionName: "7B",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["Cardek", "Ryen", "Mickaël", "Bendy1", "Sideshow Bob", "sauce"]
  },
  {
    divisionName: "7C",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: ["Marts", "Uno", "Cal", "Portalll", "mistec43", "Billy Mac"]
  },

  {
    divisionName: "8A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "Diggs",
      "PrinceFluffy",
      "Yeehaw",
      "arbaro",
      "Schtoikle",
      "Rib",
    ]
  },
  {
    divisionName: "8C",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      "SaveCatz",
      "WhiteMonster",
      "Lukideluxe",
      "SirMaser",
      "srb",
      "Samantha"
    ]
  },

  {
    divisionName: "8/9A",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 2,
    numPlayoffRelegate: 3,
    numAutoRelegate: 0,
    players: [
      "Rei",
      "doodle",
      "Yasin",
      "DocPi",
      "Sebastian",
      "Riogreywind"
    ]
  },

  {
    divisionName: "9B",
    oneMatchPerPair: true,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "mfrerck",
      "Zyild",
      "Frenchie",
      "MrShadow",
      "Junior",
      "SnowLilyx",
    ]
  }
];

const adminRole = [
  "GregBoomCannon#4087",
  "moodeuce#8573",
  "Kingsman#0364",
  "Tugi#7004",
  "BlargMeister#1337",
  "nikvo#1021"
];

const restreamerRole = [
  "moodeuce#8573",
  "Kingsman#0364",
  "Lastarot#9931",
  "Krypto#4388",
  "skiplogic#9125",
  "Chris Foryst#0131",
  "HydrantDude#2048",
  "BeastinShen#4218",
  "offwo#7439",
  "japorto#5801",
  "The1stNoel#0001",
  "Z Wing#6720",
  "DivCaste#6595",
  "vandweller#7214",
  "Tryad#0983",
  "XaeL#6732",
  "ChirpingMatt#0801",
  "Rory#1709",
  "dog#1988",
  "Kofi#6820",
  "roncli#1409",
  "EricICX#6985",
  "oktopuse#8620",
  "DitShadows#3241",
  "louieskouie#2469",
  "Johann#1501",
  "B14NK#3752",
  "Opaux (Ben)#6030",
  "Emptyeye#7326",
  "＊Chong#8809",
  "Marts#6845",
  "dogplayingtetris#9974",
  "mattholland#5596",
  "CharlesSparkley#7969",
  "ITZsharky#2353",
  "LuisZabby#2618",
  "Zipley#7137",
  "Kibi Byte#4065",
  "Tugi#7004",
  "leaurax#0455",
  "SodiumOverdose#8425",
  "c-dubs#2871",
  "NerDzzzzz#8658",
  "KAINOS#8036",
  "Vernieboi#0857",
  "TetrisTime#8026",
  "sablen#2241",
  "eggmaster#5046",
  "og_readmore#6535",
  "aGameScout#3745",
  "gummithecat#6363",
  "PukPow#6394",
  "GregBoomCannon#4087",
  "Benjy#5752",
  "Paul 楊#5532",
  "boxie3210#3078",
  "Muunas#4096",
  "ITGBalbar#2408",
  "night#4546",
  "SaltedSofa12#8996",
  "Opaux (Ben)#6969",
  "digginout#1394",
  "GDudge(UTC+2)#9821",
  "opaux#6969",
  "TotallyCalculatingPoet#8979",
  "Semilogo777#0302",
  "TotalRadNES#2445",
  "Trapzone707#2614",
  "ちどり#2119",
  "Slayer69#3183",
  "opaux#5052",
  "CombatSlayer#1987",
  "Shuram Gudatetris (UTC -5)#4111",
  "TKK24#2514",
  "Reykon#3817",
  "Ipiecevibes#6809",
  "SirMaser#1453",
  "Hepps#2169",
  "Nevanator2005#7984",
  "mil0-#1281",
  "Xerxos#5993",
  "Pkric#0281",
  "panquequeque#7897",
  "Mendion82#7313",
  "mleczak#8676",
  "Huffulufugus#9207",
  "sixfour#0130",
  "kooliooolio#1009",
  "poopyoldhag#7229",
  "Oscar._#6774",
  "Heist#9462",
  "Cwiiis#7703",
  "Elpailtem#2506",
  "SamoanGerman#2947",
  "wike#3985",
  "peyton#1996",
  "Vernie#0857",
  "odinn cube horse#1009",
  "odinn group masturbator#1009",
  "💖La Madeline💖#5443",
  "shane#9017",
  "BirbWizard#9396",
  "RIVAL | Ard#1561",
  "Zabby#2618",
  "Mr.Faq#3684",
  "Mecex#6833",
  "Apparently Glont#7898",
  "MörköS#1433",
  "Scamper9#6772",
  "Iceblade73#7051",
  "Damian-GDudge(UTC+1)#9821",
  "Spooky Samantha#8502",
  "dhks0915#7525",
  "Rhubarb#1508",
  "20Dan03#4783",
  "Jakub#4491",
  "thatguywearingblue#7417",
  "odinn group mastubator#1009",
  "scoopman#0111",
  "wide#5258",
  "Grand Designs#3638",
  "TetrisMasterRafał#0598",
  "NDDK#8648",
  "Marle#4641",
  "n0mori#6973",
  "TheMisterValor#1624",
  "pissarse#0972",
  "mel0nsoda#1438",
  "evesy#1650",
  "pumpyheart#9864",
  "RyenwithanE#5674",
  "AL#2371",
  "AlexS#9305",
  "Chillalla#6199",
  "MegaSkuci#3636",
  "Thunderstorm281#7715",
  "vanshtanwani#9639",
  "Mleczak#8676",
  "Felinho#7027",
  "Sakurion#6289",
  "Chobge#7764",
  "nikvo#1021",
  "jayp33#6638",
  "Bowser Tb#2424",
  "ian.capalot#0008",
  "🇦🇷Joacozep#0518",
  "yeehawmckickass#4099",
  "Dobry_Czlowiek#6510",
  "Lukideluxe#4491",
  "Mickaël M.#2479",
  "Burritodad#0854",
  "1231231234#1231",
  "aur9#9999",
  "Justus#4320",
  "Haven#0602",
  "Oofie#4787",
  "mxyrk#1204",
  "Felinho#3006",
  "jamestomas2#7742",
  "jakeywinks#7806",
  "ash_#7897",
  "Deewee#7730",
  "Jounce#7391",
  "SauceSavion#0030",
  "Heist#0420",
  "Threes > Twos#3333",
  "gerhard_adler#8867",
  "DerJockel#2282",
  "foiledthrice#8009",
  "Foryst#0131",
  "Somalian#9396",
  "Lord NOOM#4736",
  "mistec43#9728",
  "melhk#4155",
  "AvGeekW159#0616",
  "Adrena1in#3056",
  "Xenophilius#9730",
  "Gabriel B#0072",
  "Heist#8449",
  "FrenchiestFrie#9778",
  "Fidhapre#9453",
  "KunfusingPoke#6098",
  "Twifty#3688",
  "UnOr1ginalName#8655",
  "minertyler100#0369",
  "Akideemus#0151",
  "Bakadrian#0601",
  "Damian-GDudge(UTC+2)#9821",
  "SARGIN#9722",
  "DavidMichaelJ#5419",
  "Alex Thach#0648",
  "zydras#9476",
  "Richard Wolf VI#3522",
  "Noriko#0030",
  "Sleepy Samantha#8502",
  "PolloSalazar#1503",
  "rens#5484",
  "NNNNNNNNNNNNNN#2430",
  "AkumaLuxray#4503",
  "cmg444\\#4652",
  "Dorukkaptan95#0404"
];

module.exports = {
  divisionData,
  adminRole,
  restreamerRole
};
