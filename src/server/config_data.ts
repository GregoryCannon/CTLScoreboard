import type {
  CompetitionSettings,
  DivisionSettings
} from "../types.ts";

const divisionData: DivisionSettings[] = [
  {
    divisionName: "1A",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    bestOf: 5,
    maxPointsPerMatch: 7,
    numWinner: 1,
    numPrizeMoney: 2,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      'Coal',
      'Cobra',
      'Hydrant',
      'Somalian',
      'O',
      'Sidnev',
      'SillyShark',
    ]
  },
  {
    divisionName: "1B",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 1,
    numPrizeMoney: 4,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      'Nek0',
      'Cryticalapis',
      'Gwafey',
      'ID Kirby',
      'Phloxae',
      'Sam Vaughan',
      'TetrisTime',
    ]
  },

  {
    divisionName: "2A",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    bestOf: 5,
    maxPointsPerMatch: 7,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      'Crabrangoon',
      'DuzyK',
      'Marq',
      'Shake',
      'Shqdwnight',
      'Winterra',
    ]
  },
  {
    divisionName: "2B",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    bestOf: 5,
    maxPointsPerMatch: 7,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      'Alejo',
      'Night',
      'Evesy',
      'Smoljordan',
      'Tyler',
      'Lokenze',
    ]
  },
  

  {
    divisionName: "3A",
    completed: false,
    competition: "ctl",
    oneMatchPerPair: true,
    bestOf: 5,
    maxPointsPerMatch: 7,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 2,
    numAutoRelegate: 0,
    players: [
      'Anpu',
      'Doodle',
      'Guerrero',
      'Mari',
      'PacoisTaco',
      'Scamper',
      'ThisinThat',
    ]
  },
  {
    divisionName: "3B",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 2,
    numAutoRelegate: 0,
    players: [
      '20Dan03',
      'Birbia',
      'Firedox',
      'Gresiu',
      'Jati',
      'Oscar Godinez',
      'TeainRain',
    ]
  },

  {
    divisionName: "4A",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      'AFMirror',
      'Cyler',
      'Da3mon',
      'Kannplaystetris',
      'MLGCaptainVirgil',
      'Mr.Faq',
    ]
  },
  {
    divisionName: "4B",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      'Draco',
      'Grand Designs',
      'Jojo',
      'Morelando',
      'Spectric',
      'Whitemonster',
    ]
  },
  {
    divisionName: "4C",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      'Anjing',
      'Jay',
      'JediEvan',
      'moodeuce',
      'Toka',
      'WDSubia',
    ]
  },
  {
    divisionName: "5A",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      'Klovelace',
      'Roma',
      'Sooshimanh',
      'Tacit',
      'Tired',
      'Unskill',
      'Vasco',
      'Bran',
    ]
  },
  {
    divisionName: "5B",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      'Decklad',
      'EvanJ',
      'Kingsman',
      'Ravioli',
      'Roman',
      'Roncli',
      'Snowytetris',
      'Mantaskutop',
    ]
  },
  {
    divisionName: "5C",
    competition: "ctl",
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 7,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 1,
    players: [
      'Barry',
      'Hecate',
      'Iceblade73',
      'Letcreate',
      'M149',
      'Mathwiz',
      'StolenShortSword',
      'Talvot',
    ]
  },

  // END CTL

  {
    divisionName: "Roentgenium 1",
    competition: "tnp",
    completed: true,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 2,
    numAutoPromo: 0,
    numPrizeMoney: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "TEAFORDINNER",
      "M149",
      "LOKENZE",
      "MIKAWAY",
      "ANJING",
      "ICEBLADE",
    ]
  },
  {
    divisionName: "Roentgenium 2",
    competition: "tnp",
    completed: true,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 2,
    numAutoPromo: 0,
    numPrizeMoney: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "JOJO",
      "GUSTAVO",
      "WDSUBIA",
      "THISINTHAT",
      "DOBRY",
    ]
  },
  {
    divisionName: "Roentgenium 3",
    competition: "tnp",
    completed: true,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 2,
    numAutoPromo: 0,
    numPrizeMoney: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "JEDIEVAN",
      "TOKA",
      "LIPTONEQ",
      "SNOWYTETRIS",
      "GRANDDESIGNS",
      "PENGUINPANIC",
      "SERENITYINSORROW",
    ]
  },
  {
    divisionName: "Roentgenium 4",
    competition: "tnp",
    completed: true,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "BRIANTHEONE",
      "THISINTHAT",
      "ANJING",
      "SPECTRIC",
      "KANNPLAYSTETRIS",
      "STOLENSHORTSWORD",
      "BLINGUS",
      "MOODEUCE",
      "ICEBLADE73",
      "HECATE",
      "PENGUINPANIC",
      "MATHWIZ",
      "ZENON",
      "JEDIEVAN",
      "GUSTAVOCUBISTA7",
      "UNOR1GINALNAME",
    ]
  },
  {
    divisionName: "Roentgenium 5",
    competition: "tnp",
    completed: true,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "SPECTRIC",
      "MATHWIZ",
      "TIRED_RL",
      "MOODEUCE",
      "SUPERJAYCODER",
      "M149",
      "ZENON",
      "ANJING",
      "SNOWYTETRIS",
      "VASCO",
      "MORELANDO",
      "ICEBLADE73",
      "JEDIEVAN",
      "BRANGUY",
      "AFMIRROR",
    ]
  },
  {
    divisionName: "Roentgenium 6",
    competition: "tnp",
    completed: false,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "SNOWYTETRIS",
      "KANNPLAYSTETRIS",
      "RAVIOLI",
      "SPECTRIC",
      "TIRED_RL",
      "MATHWIZ",
      "M149",
      "MOODEUCE",
      "WHITE MONSTER",
      "EZHEDEH",
      "20DAN03",
      "JEDIEVAN",
      "ZENON",
      "ICEBLADE73",
      "VASCO",
      "DOBRYCZLEK",
      "MORELANDO",
      "LILIANA",
    ]
  },

  // BEGIN GOLD

  {
    divisionName: "Gold 64",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: ["KLOVELACE", "MANTASKUTOP", "SWIDR", "VASCO", "EZHEDEH"]
  },
  {
    divisionName: "Gold 65",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "KINGSMAN",
      "ETHANTHOUGHTS",
      "SUPERJAYCODER",
      "SPECTRIC",
      "KHAM",
      "ARBARO",
      "RORY"
    ]
  },
  {
    divisionName: "Gold 66",
    competition: "tnp",
    completed: true,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "LILIANA",
      "SOOSHIMANH",
      "STOLENSHORTSWORD",
      "KLOVELACE",
      "RORY",
      "CHEFBOYARGEEBS",
      "SEBIE"
    ]
  },
  {
    divisionName: "Gold 67",
    competition: "tnp",
    completed: true,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "VASCO",
      "KINGSMAN",
      "PETERTHECHAIR",
      "SUPERJAYCODER",
      "RORY",
      "LILIANA",
      "KANNPLAYSTETRIS",
      "ETHANTHOUGHTS"
    ]
  },
  {
    divisionName: "Gold 68",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "MARTS",
      "STOLENSHORTSWORD",
      "LILIANA",
      "TIRED_RL",
      "COVERDALES",
      "KLOVELACE"
    ]
  },
  {
    divisionName: "Gold 69",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "BARRYBLANKET",
      "MANTASKUTOP",
      "RORY",
      "KINGSMAN",
      "HUNTERZZZZ",
      "ENSCALIGHT",
      "ETHANTHOUGHTS",
    ]
  },
  {
    divisionName: "Gold 70",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "SUPERJAYCODER",
      "TIRED_RL",
      "SNOWYTETRIS",
      "LILIANA",
      "M149",
    ]
  },
  {
    divisionName: "Gold 71",
    competition: "tnp",
    numPrizeMoney: 0,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    bestOf: 5,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "MAEDHROS",
      "MARTS",
      "KOCCHIRA",
      "CHEFBOYARGEEBS",
      "BRAN"
    ]
  },
  {
    divisionName: "Gold 72",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "M149",
      "MANTASKUTOP",
      "MARTS",
      "THATKIDKIM",
      "ROMA",
      "PETERTHECHAIR",
      "LILIANA",
    ]
  },
  {
    divisionName: "Gold 73",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "SNOWYTETRIS",
      "ROMA",
      "MARTS",
      "LILIANA",
      "PUMMYZOI",
    ]
  },
  {
    divisionName: "Gold 74",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "CHEFBOYARGEEBS",
      "GRAVEL1942",
      "MARTS",
      "KLOVELACE",
      "LILIANA",
      "HUNN14",
    ]
  },
    {
    divisionName: "Gold 75",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "THATKIDKIM",
      "ANBER",
      "MAEDHROS",
      "MIKAWAY",
      "MAXXF386",
    ]
  },
  {
    divisionName: "Gold 76",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "KLOVELACE",
      "PETERTHECHAIR",
      "PUMMYZOI",
      "MARTS",
    ]
  },
  {
    divisionName: "Gold 77",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "SEBIE",
      "LOUCY",
      "LILIANA",
      "GBTOAST",
    ]
  },
  {
    divisionName: "Gold 78",
    competition: "tnp",
    completed: false,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "MIKAWAY",
      "LUKIDELUXE",
      "MARTS",
      "PUMMYZOI",
      "LOUCY",
      "NIA",
      "BILLYMAC",
      "GBTOAST",
    ]
  },
  {
    divisionName: "Gold 79",
    competition: "tnp",
    completed: false,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "FELIX",
      "MAEDHROS",
      "MARTS",
      "EMOKID",
      "ARBARO",
      "PETERTHECHAIR",
    ]
  },
  {
    divisionName: "Gold 80",
    competition: "tnp",
    completed: false,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "MANTASKUTOP",
      "KOMI",
      "ADAMIRISH",
      "TYOW",
      "KASANETETO123",
    ]
  },
  {
    divisionName: "Gold 81",
    competition: "tnp",
    completed: false,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "MFRERCK",
      "LUKIDELUXE",
      "RAVALO",
      "HECATE",
      "321MRHAATZ",
    ]
  },
  
  // END GOLD
  // BEGIN SILVER

  {
    divisionName: "Silver 74",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numPrizeMoney: 0,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "CHEFBOYARGEEBS",
      "LILIANA",
      "TOAST",
      "UQI",
      "THATKIDKIM",
      "RACHIT",
      "MIRRORRIKER"
    ]
  },
  {
    divisionName: "Silver 76",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numPrizeMoney: 0,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "YASMINA",
      "TEMPHUIBIS",
      "DIAMONDS",
      "GRAVEL",
      "SUPERJAYCODER",
      "MARTS",
      "MAXXF386",
      "JUST_WOBERT"
    ]
  },
  {
    divisionName: "Silver 77",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numPrizeMoney: 0,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "ERILIONESS",
      "FURIO",
      "ENSCALIGHT",
      "GILDEDLIZARD",
      "THATKIDKIM",
      "KALIMOTXO",
      "ADAMIRISH",
      "GBTOAST"
    ]
  },
  {
    divisionName: "Silver 78",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numPrizeMoney: 0,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "ARBARO",
      "IZOBREEN7",
      "TIRED_RL",
      "GRAVEL1492",
      "DIAMONDS",
      "SUPERNINJA",
      "GILDEDLIZARD"
    ]
  },
  {
    divisionName: "Silver 79",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numPrizeMoney: 0,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "ADAMIRISH",
      "KAT_5528",
      "MAXXF386",
      "SUPERSTEP",
      "GBTOAST",
    ]
  },
  {
    divisionName: "Silver 80",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "ALICE_FERN",
      "RAVIOLI",
      "GRAVEL1942",
      "LUKIDELUXE",
      "ARBARO",
    ]
  },
  {
    divisionName: "Silver 81",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "TEMPHUIBIS",
      "ADAMIRISH",
      "LOUCY",
      "GILDEDLIZARD",
      "KALIMOTXO",
      "MAXXF386",
      "LUKIDELUXE",
      "SUPERNINJA",
    ]
  },
  {
    divisionName: "Silver 82",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "CYLER",
      "RAVIOLI",
      "TYOW",
      "ASHLEYYYY",
      "FURIO",
      "DANNYMAGE",
    ]
  },
    {
    divisionName: "Silver 83",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "LUKIDELUXE",
      "ARBARO",
      "KASANETETO123",
      "TEMPHUIBIS",
      "ADAMIRISH",
      "JUST_WOBERT",
      "SUPERNINJA",
    ]
  },
  {
    divisionName: "Silver 84",
    competition: "tnp",
    completed: true,
    bestOf: 7,
    oneMatchPerPair: true,
    maxPointsPerMatch: 10,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "TYOW",
      "ERILIONESS",
      "IDUDHJF",
      "GERT",
      "GENIALERGANDALF",
      "FALSEMINUTE",
      "TALE3339",
      "ASHLEYYYY",
    ]
  },
  {
    divisionName: "Silver 85",
    competition: "tnp",
    completed: true,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "KASANETETO123",
      "DANNYMAGE",
      "VINZCHILLIN",
      "ADAMIRISH",
      "ASHLEYYYY",
      "FURIO",
    ]
  },
  {
    divisionName: "Silver 86",
    competition: "tnp",
    completed: false,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "ALICE",
      "RANCE",
      "ARISZ",
      "SUPERSTEP",
      "GERT",
      "MIRRORRIKER",
    ]
  },

  // END SILVER
  // BEGIN BRONZE

  {
    divisionName: "Bronze 82",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: ["ERILIONESS", "T1NY", "LEYMBDA", "WEEGLOOGI", "MS_DROP"]
  },
  {
    divisionName: "Bronze 83",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "ASHLEYYYY",
      "CROIOS",
      "ELLIOTHL1",
      "MASHINGSAPPHIC",
      "BBOT",
      "GILDEDLIZARD",
      "SUPERSTEP",
      "NERDYFELIX"
    ]
  },
  {
    divisionName: "Bronze 85",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "TIRED_RL",
      "RAVIOLI",
      "WILSON",
      "ASTRID",
      "VENT",
      "MIZORE",
      "TYOW"
    ]
  },
  {
    divisionName: "Bronze 86",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "GRAVEL1492",
      "ASHLEYYYY",
      "TIFFYFLOWR",
      "ELLIOT",
      "LEYMBDA",
      "SUPERSTEP",
      "BIRBIA",
      "WEEGLOOGI"
    ]
  },
  {
    divisionName: "Bronze 87",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: ["LOUCY", "WILSON", "CONDENSED_ARIN", "RAVIOLI", "TYOW", "SIERRAE"]
  },
  {
    divisionName: "Bronze 88",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "LADYVADER",
      "LEYMBDA",
      "YAMA",
      "KRYPTON",
      "WEEGLOOGI",
      "SWAGGIE",
      "BEAN",
      "POSSUMBAKES",
    ]
  },
  {
    divisionName: "Bronze 89",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "WILSON",
      "TYOW",
      "RATTAILEDJAGUAR",
      "ASHLEYYYY",
      "COXDANNYJ",
      "GENIALERGANDALF",
      "BBOT",
    ]
  },
  {
    divisionName: "Bronze 90",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "CONDENSED_ARIN",
      "CROIOS",
      "FALSEMINUTE",
      "EXITY",
      "VENT",
      "THETIGERSHARK",
      "JUST_JOE",
    ]
  },
  {
    divisionName: "Bronze 91",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "TALVOT",
      "FALSEMINUTE",
      "GENIALERGANDALF",
      "JDHCDABOI",
      "LEYMBDA",
    ]
  },
  {
    divisionName: "Bronze 92",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "SUPERSTEP",
      "IDUDHJF",
      "DJANKOFF",
      "RANCE",
      "CONDENSED_ARIN",
    ]
  },
  {
    divisionName: "Bronze 93",
    competition: "tnp",
    completed: false,
    bestOf: 5,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPrizeMoney: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "CALEBSGAMESANDSTUFF",
      "WILSON",
      "TALE",
      "VINZCHILLIN",
      "BLOODLUSTFAIL",
    ]
  },
    {
    divisionName: "Bronze 94",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "MANSION",
      "WILSON",
      "DJANKOFF",
      "LEYMBDA",
      "SUPERSTEP",
    ]
  },
  {
    divisionName: "Bronze 95",
    competition: "tnp",
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "CONDENSED_ARIN",
      "RANCE",
      "CALEBSGAMESANDSTUFF",
      "ANDALUSIAN_RG",
      "BLOODLUSTFAIL",
    ]
  },
  {
    divisionName: "Bronze 96",
    competition: "tnp",
    completed: false,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
    "DAZZELOIDS",
    "KRYPTON",
    "DARK_SHADOW",
    "ASLEEPIEALLIE",
    "MANSION",
    ]
  },
  {
    divisionName: "Bronze 97",
    competition: "tnp",
    completed: false,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    bestOf: 5,
    numWinner: 0,
    numAutoPromo: 1,
    numPrizeMoney: 0,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "BLOODLUSTFAIL",
      "CONDENSED_ARIN",
      "WILSON",
      "YAMA",
      "ROGUEPOTATO",
      "LEYMBDA",
    ]
  },
  
  // END BRONZE
  // BEGIN BEGINNER

  {
    divisionName: "Elm 15",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "CONDENSED_ARIN",
      "KURDIGAMER",
      "SWIFTSLAYER",
      "UNNAMED USER",
      "THEONLYCEDRIC",
      "MRBRAIN",
      "EXITY"
    ]
  },
  {
    divisionName: "Elm 16",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "ASLEEPIEALLIE",
      "MRBRAIN",
      "52KANON",
      "EXITY",
      "SAYMYNAME"
    ]
  },
  {
    divisionName: "Elm 17",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "VINZCHILLIN",
      "KIMBERLYSAYS",
      "THETIGERSHARK",
      "JDHCDABOI",
    ]
  },
  {
    divisionName: "Elm 18",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "DAZZELOIDS",
      "ASLEEPIEALLIE",
      "VINZCHILLIN",
      "MRBRAIN",
    ]
  },
  {
    divisionName: "Elm 19",
    competition: "tnp",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: false,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 0,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "TALE3339XD",
      "SAKURAINBLOOM",
      "ASLEEPIEALLIE",
      "VINZCHILLIN",
    ]
  },
  {
    divisionName: "Elm 20",
    competition: "tnp",
    completed: false,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numAutoPromo: 0,
    numPrizeMoney: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "DAZZELOIDS",
      "THEONLYCEDRIC",
      "ASLEEPIEALLIE",
      "GODATTETRIS",
    ]
  },
  {
    divisionName: "Elm 21",
    competition: "tnp",
    completed: false,
    oneMatchPerPair: false,
    bestOf: 5,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numAutoPromo: 0,
    numPrizeMoney: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "DGORDON888",
      "GUNENG24",
      "THEONLYCEDRIC",
      "KANON",
    ]
  },


  // END TNP
  // BEGIN DAS LEAGUE

  {
    divisionName: "DL-S4-1A",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 3,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "ALEX T",
      "VANCE",
      "AVGEEK",
      "BENDY1",
      "TOMMYNTG",
      "BLUE SCUTI",
    ]
  },
  {
    divisionName: "DL-S4-1B",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 3,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "WALLBANT",
      "SALTEDSOFA",
      "ROBIN",
      "TEGAMECH",
      "LIOLIO",
      "GIOVANNI",
    ]
  },
  {
    divisionName: "DL-S4-2",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 2,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "SHUICHI",
      "USERNAMEGOLD",
      "POOPYOLDHAG",
      "IANKEITH",
      "WEISBROT",
      "JEDIEVAN",
      "SOMALIAN"
    ]
  },
  {
    divisionName: "DL-S4-3",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "PENGUINPANIC",
      "ADAMANATITE",
      "ANGRYRORO",
      "GUERREROASDF",
      "FIREDOX",
      "MORELANDO",
      "MARI"
    ]
  },
  {
    divisionName: "DL-S4-4",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "MIKAWAY",
      "THISINTHAT",
      "SUPERJAYCODER",
      "HECATE",
      "POO_DIEPIO",
      "Z0HASSADAR",
      "LOKENZE"
    ]
  },
  {
    divisionName: "DL-S4-5",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "SNOWYTETRIS",
      "TIRED_RL",
      "ARBARO",
      "20DAN03",
      "BIGBOI",
      "SWIDR",
    ]
  },
  {
    divisionName: "DL-S4-6",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "GBTOAST",
      "MADDY",
      "BRIANTHEONE",
      "DANNYMAGE",
      "CONDENSED_ARIN",
      "ANJING",
    ]
  },

  {
    divisionName: "DL-S5-1A",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 3,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "ZACHGAMESWELL",
      "USERNAMEGOLD",
      "ROBIN",
      "SALTEDSOFA",
      "LIOLIO",
      "WALLBANT",
    ]
  },
  {
    divisionName: "DL-S5-1B",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 3,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "IANKEITH",
      "TEGAMECH",
      "ELIAS",
      "BENDY1",
      "BLUE SCUTI",
    ]
  },
  {
    divisionName: "DL-S5-2A",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "SOMALIAN",
      "RURCZAK",
      "CURI",
      "COBRA",
      "POOPYOLDHAG",
      "SHUICHI",
      "GIOVANNI"
    ]
  },
  {
    divisionName: "DL-S5-2B",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 0,
    players: [
      "FIREDOX",
      "JEDIEVAN",
      "STEELCE",
      "OSCAR",
      "VANCE",
      "MASY"
    ]
  },
  {
    divisionName: "DL-S5-3",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 2,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "LOKENZE",
      "NERDTHEBOX",
      "RXS",
      "ANGRYRORO",
      "POO_DIEPIO",
      "DEEWEE",
      "YELENA",
      "GUNTER"
    ]
  },
  {
    divisionName: "DL-S5-4",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "THISINTHAT",
      "MOODEUCE",
      "M149",
      "MIKAWAY",
      "TRAPZONE",
      "CATVINITY",
      "SWIDR",
      "SNOWYTETRIS"
    ]
  },
  {
    divisionName: "DL-S5-5",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 2,
    numAutoRelegate: 0,
    players: [
      "KINGSMAN",
      "SUPERJAYCODER",
      "BARRYBLANKET",
      "TIRED_RL",
      "LILIANA",
      "ANJING",
      "GBTOAST"
    ]
  },
  {
    divisionName: "DL-S5-6A",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "WILSON",
      "MAEDHROS",
      "CRESPO",
      "ARBARO",
      "DOBRYCZLEK"
    ]
  },
  {
    divisionName: "DL-S5-6B",
    competition: "dl",
    bestOf: 5,
    completed: true,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "TALE",
      "SPECTRIC",
      "MARTS",
      "DANNYMAGE",
      "KYLA"
    ]
  },

  {
    divisionName: "DL-S6-1A",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 3,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
		  "CURI",
      "RURCZAK",
      "MICETRO",
      "IANKEITH",
      "SALTEDSOFA",
      "NIGHT",
      "TEGAMECH",
      "TUGI",
    ]
  },
  {
    divisionName: "DL-S6-1B",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 1,
    numPrizeMoney: 3,
    numAutoPromo: 0,
    numPlayoffPromo: 0,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "FIREDOX",
      "STEELCE",
      "VANCE",
      "CHROMA",
      "SHUICHI",
      "SCOTTOBOZO",
      "BENDY",
      "GOLD",
    ]
  },
  {
    divisionName: "DL-S6-2",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 2,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "ZACHGAMESWELL",
      "WALLBANT",
      "MASY",
      "NERDTHEBOX",
      "ANPU",
      "POOPYOLDHAG",
      "JDH",
      "MARI",
      "SOMALIAN",
    ]
  },
  {
    divisionName: "DL-S6-3",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 2,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "WEISBROT",
      "JANI",
      "GIOVANNI",
      "JAY",
      "TIRED",
      "ROMAN",
      "GUERRERO",
      "MARQ",
      "TRAPZONE",
    ]
  },
  {
    divisionName: "DL-S6-4",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 2,
    numPlayoffPromo: 2,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "ELPAILTEM",
      "BARRYBLANKET",
      "MORELANDO",
      "HEPTA",
      "YUSIF",
      "M149",
      "SWIDR",
      "SNOWYTETRIS",
      "CATVINITY",
    ]
  },
  {
    divisionName: "DL-S6-5A",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "MARTS",
      "CRESPO",
      "OMENVOIR",
      "RAVALO",
      "MIKAWAY",
      "ZOHASSADAR",
    ]
  },
  {
    divisionName: "DL-S6-5B",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 1,
    numPlayoffRelegate: 1,
    numAutoRelegate: 1,
    players: [
      "ANJING",
      "KANN",
      "20DAN03",
      "LILIANA",
      "EEE_INN",
      "GBTOAST",
    ]
  },
  {
    divisionName: "DL-S6-6",
    competition: "dl",
    bestOf: 5,
    completed: false,
    oneMatchPerPair: true,
    maxPointsPerMatch: 8,
    numWinner: 0,
    numPrizeMoney: 0,
    numAutoPromo: 1,
    numPlayoffPromo: 2,
    numPlayoffRelegate: 0,
    numAutoRelegate: 0,
    players: [
      "GERT",
      "KINGSMAN",
      "MATHWIZ",
      "ALICE FERN",
      "ERILIONESS",
      "SEVENPAGEWILSON",
    ]
  },




];

const competitions: CompetitionSettings[] = [
  {
    abbreviation: "ctl",
    buttonName: "CTL",
    eloName: "CT League",
    registrationBotInfo: {
      registrationOpen: true,
      divisions: ["1", "2", "3", "4", "5"],
      signupMessage:
        "Signups for upcoming CTL seasons will be announced at the end of each previous season and will be closed a week before the start of the next one. Click the appropriate reaction for the tier level you are assigned to if you wish to participate. Should you need to withdraw after the CTL draw takes place, notify the King immediately so a replacement may be found, if necessary. For full details, refer to #rules-and-standings.\n\n",
      registrationClosedMessage:
        "Signups for the current season are now closed. Please wait for an announcement later in the season for when you may begin to react to participate for the next season.",
    }
  },
  {
    abbreviation: "tnp",
    buttonName: "TNP",
    eloName: "TNP",
    registrationBotInfo: {
      registrationOpen: true,
      divisions: ["Gold", "Silver", "Bronze", "Beginner", "Roentgenium"],
      signupMessage: `
        React to sign up for a new division below! Your reaction will be hidden after 3 seconds.

        If you are currently in a division, do not sign up here until both (1) 100% of your own matches have been completed, and (2) your division deadline is at the end of the current week or your tier for the next season is already a 100% certainty. Make sure you sign up for the correct tier, and if you want to withdraw, do so before 23:59 UTC on Monday. New divisions will be created shortly thereafter and withdrawal after this point may be penalised. For full details, refer to #about.`,
      registrationClosedMessage: "Signups are now closed."
    }
  },
  {
    abbreviation: "dl",
    buttonName: "DAS League",
    eloName: "DAS League",
    registrationBotInfo: {
      registrationOpen: false,
      divisions: ["1", "2", "3", "4", "5", "6"],
      signupMessage: "React to signup below :)\n",
      registrationClosedMessage: "Signups are now closed."
    }
  },
]


const adminRole: string[] = [
  "gregboomcannon#0",
  "moodeuce#0",
  "kingsman36#0",
  "tugi#0",
  "notzempro#0",
  "pkric#0",
  "danvvv#0",
  "nikvo#0",
  "chobge#0",
  "chillalla#0",
  "richardwolfvi#0",
  "lilhana4#0",
  "snowytetris#0",
];

const restreamerRole: string[] = [
  ".charsiu.#0",
  ".hazelnutmilk.#0",
  ".yler#0",
  "20dan03#0",
  "321mrhaatz#0",
  "7pagewilson#0",
  "8mansion8#0",
  "aanti#0",
  "adamirish#0",
  "afmirror#0",
  "agamescout#0",
  "akatu#0",
  "alexthach#0",
  "alice_fern#0",
  "allenbot#0",
  "allpowerfulorange#0",
  "anangryroro#0",
  "angrytrashyperson#0",
  "aptiz712#0",
  "asherfullback#0",
  "asleepyallie#0",
  "augmenttheampwave#0",
  "avgeekw159#0",
  "avriix#0",
  "b0rkedbear#0",
  "bendy1#0",
  "bergtroll#0",
  "billymac_#0",
  "birbia#0",
  "birbwizard#0",
  "birbia#0",
  "blank0432#0",
  "blargmeister#0",
  "blingus#0",
  "boppyreal#0",
  "boxie3210#0",
  "brantheepic#0",
  "bunnyboing#0",
  "burritodad#0",
  "catsugiri#0",
  "catvinity#0",
  "cchristm#0",
  "cdoug17#0",
  "chefboyargeebs#0",
  "chillala#0",
  "chobge#0",
  "cmcajaba#0",
  "coalbucket2#0",
  "cobra6731#0",
  "codecracker01#0",
  "cooltouchdown#0",
  "cousin_avi33#0",
  "coverdales4#0",
  "coxdannyj#0",
  "cwiiis#0",
  "cylersrc#0",
  "dannymage#0",
  "danvvv#0",
  "deepshock#0",
  "deewee7#0",
  "dekk_cl#0",
  "dobreee#0",
  "docpi314#0",
  "dogpt#0",
  "drkn0ckers#0",
  "dylan297#0",
  "eliwa#0",
  "elpailtem#0",
  "enscalight#0",
  "eri_lioness#0",
  "ethan1405#0",
  "eve_d#0",
  "ezhedeh#0",
  "falkite#0",
  "firedox#0",
  "frenchiestfrie#0",
  "frodododo#0",
  "gabrielbondon#0",
  "gbtoast#0",
  "gekki.jb#0",
  "gerhard_adler#0",
  "gildedlizard#0",
  "gregboomcannon#0",
  "hai5772#0",
  "heist#0",
  "homejacob#0",
  "huffulufugus#0",
  "hunn14#0",
  "hunter.four.z.#0",
  "hydrantdude#0",
  "iceblade73#0",
  "invertedthewing#0",
  "itzsharky#0",
  "iuqe#0",
  "jacksonthebest#0",
  "jati727#0",
  "javantea#0",
  "jayissuperawesome1#0",
  "jedievan4#0",
  "jxxojo#0",
  "jrgagne99#0",
  "just_wobert#0",
  "kalimotxojm#0",
  "kat_#0",
  "kingsman36#0",
  "klovelace10#0",
  "kominestris#0",
  "kyira#0",
  "ladyvader13_43642#0",
  "lanheg#0",
  "lia_fr0st#0",
  "lilhana4#0",
  "lilian7774#0",
  "liptoneq196#0",
  "lokenze#0",
  "lukideluxe#0",
  "luphoria#0",
  "m145#0",
  "m149#0",
  "maedhros1978#0",
  "majoneee#0",
  "marfram#0",
  "mariachilocot#0",
  "marts6845#0",
  "mathmaster13#0",
  "mathwiz100#0",
  "maxwu27#0",
  "mcfloatlover#0",
  "me_in_2023#0",
  "mel0nsoda#0",
  "meltedplague856#0",
  "mendion#0",
  "mfrerck#0",
  "micetro#0",
  "mikdagreat#0",
  "minertyler100#0",
  "minikey1219#0",
  "moodeuce#0",
  "morelando_#0",
  "mqisie#0",
  "mrfaq#0",
  "neutrisicacid#0",
  "nikvo#0",
  "ninjaofninjas5#0",
  "notleo._.#0",
  "notzempro#0",
  "omenvoir#0",
  "p1_carter#0",
  "peterthechair#0",
  "peyton1996#0",
  "pkric#0",
  "pohonjat1#0",
  "poopyoldhag#0",
  "poteznymleczarz#0",
  "pumpyheart#0",
  "r3io4mf#0",
  "r75mm#0",
  "ravioli1301#0",
  "roguepotatobaduk#0",
  "richardwolfvi#0",
  "rockdtben#0",
  "roncli#0",
  "roryrorysupermarket#0",
  "salexm#0",
  "saltedsofa12#0",
  "scamper9#0",
  "sebie_1066#0",
  "shackman04#0",
  "shoppingtrolli#0",
  "sidnev#0",
  "sirmaser#0",
  "sleebymaddy#0",
  "snowytetris#0",
  "sonicthescrew#0",
  "spec978#0",
  "stolenshortsword#0",
  "storminorman#0",
  "suds1821#0",
  "superjaycoder#0",
  "superninja9999#0",
  "swagj13#0",
  "tacomom18#0",
  "talvot21#0",
  "teainrain#0",
  "temphuibis#0",
  "tetrisroma#0",
  "theianwright#0",
  "thecresp#0",
  "thelovasman#0",
  "thisinthat#0",
  "threestwos#0",
  "thund7#0",
  "tiffyflowr#0",
  "timmykim915#0",
  "trapzone707#0",
  "treepriest#0",
  "tstorm281#0",
  "tugi#0",
  "tweak_beacon#0",
  "tyow#0",
  "unor1ginalname#0",
  "vachetbeasty#0",
  "vinzchillin#0",
  "visor702#0",
  "w___i___d___e#0",
  "wally4426#0",
  "wdsubia#0",
  "weston6673#0",
  "winterra#0",
  "wyliecantdischord#0",
  "xenoboots#0",
  "yeehawmckickass#0",
  "zxvye.#0",
  "zydras07#0"
];

export {
  divisionData,
  adminRole,
  restreamerRole,
  competitions
};
