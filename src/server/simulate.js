...
function didClinchPromo(targetPlayer, division){
  const divisionStandings = simulateWorstSeasonForPlayer(division, targetPlayer);
  let promoed = false;
...
function didClinchNonPromo(player, division){
  ...
  // Auto promo or win
  for (let i = 0; i < division.numAutoPromo + division.numWinner + division.numSoftPromo; i++) {
    const promoedName = divisionStandings[i].name;
...
function runSimulation(division, matchSchedule) {
  ...
  for (let p = 0; p < division.players.length; p++) {
    ...
    } else if (player.promoChance == 0 && !didClinchNonPromo(player, division)){
      player.promoChance = 0.01;
    }
    if (player.relegationChance == 100 && !didClinchRelegation(player, division)){
      player.relegationChance = 99.99;
    } else if (player.relegationChance == 0 && !didClinchNonRelegation(player, division)){
