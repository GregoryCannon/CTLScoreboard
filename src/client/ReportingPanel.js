...
class ReportingPanel extends Component {
  ...
  changeWinnerHome(isHome) {
    console.log("ChangeWinnerHome called:", isHome);
  ...
  submitClicked() {
    ...
    request.open("POST", "http://localhost:8080/match-data", true);
      ...
      <div>
        ...
        {this.state.reportingDivision !== "" ? (
          <div className="Match-reporting-container">
            {/* Winner */}
            <div className="Reporting-panel-container">
              <select id="winner-name" onChange={this.changeWinner}>
            ...
            <div className="Reporting-panel-container">
              <select id="loser-name" onChange={this.changeLoser}>
                <option value="" selected disabled>
                  (loser)
                </option>
                {playerNameList.map((playerName, i) => {
